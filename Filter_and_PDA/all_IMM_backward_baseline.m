classdef all_IMM_backward_baseline < handle
...
%% functions of IMM
    methods
        ...
        function this = all_IMM_backward_baseline(filters, initialState, move_cov_est_initial, betas, thetas, mass, startTime, endTime)
...
%             fprintf('time:%d -> %d\n',time,time+deltaTime*10);
          ...
          %% parameter setting for base filters
            ...
            for b_idx = 1:length(this.betas)
                for t_idx = 1:length(this.thetas)
                    for m_idx = 1:length(this.mass)
                        this.filters{length(this.betas)^2*(b_idx-1)+(length(this.thetas)^1)*(t_idx-1)+(length(this.mass)^0)*(m_idx-1) + 1}.paramF(1) = beta(b_idx);
                        this.filters{length(this.betas)^2*(b_idx-1)+(length(this.thetas)^1)*(t_idx-1)+(length(this.mass)^0)*(m_idx-1) + 1}.paramF(2) = theta(t_idx);
                        this.filters{length(this.betas)^2*(b_idx-1)+(length(this.thetas)^1)*(t_idx-1)+(length(this.mass)^0)*(m_idx-1) + 1}.paramF(3) = mass(m_idx);
                    end
                end
            end
          ...
          %% OUTPUT : Return Purpose
            ...
            params = {};          
            total_beta = reshape(cat(1, beta, beta, beta, beta, beta, beta, beta, beta, beta), 1, this.numFilters) * mu_k_1;
            total_theta = repmat(reshape(cat(1, theta, theta, theta), 1, this.numFilters/length(theta)), 1, 3) * mu_k_1;
            total_mass = repmat(mass, 1, this.numFilters/length(mass)) * mu_k_1;
            
            params{1} = total_beta;
            params{2} = total_theta;
            params{3} = total_mass;
