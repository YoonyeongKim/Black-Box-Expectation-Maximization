classdef all_UKF_backward_baseline < handle
...
%% functions of UKF
    methods
       %% func1. Initialization
        function this = all_UKF_backward_baseline(F,paramF,H,paramH,initialState,move_cov_est_initial,observationSize,alpha,beta,kappa,gamma)
       ...
       %% func3. Prediction step
        function [x_propagated,move_cov_propagated,y_propagated,measure_cov_est,cross_cov_est] = runFilterPrediction(this,time,deltaTime,measure_cov,x_est_imm,move_cov_est_imm)
          ...
          %% STEP 2 : Prediction
            ...
            for j = 1:(2*this.D+1)
                x_samplePoints_propagated(:,j) = feval(this.F, x_samplePoints(:,j), time*0.1, deltaTime, this.paramF(1), this.paramF(2), this.paramF(3));
            end
